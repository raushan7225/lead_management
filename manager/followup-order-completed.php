...
$lead_list = "SELECT * FROM `leads` WHERE `lead_for_company`='$Company' AND `lead_for_branch`='$Branch' AND `lead_assign_to`!='' AND `lead_updatedby`!='' AND `lead_type` = 'Completed' ORDER BY `id` DESC";
        ...
        const leadData = [
            ...
            if ($counterValue <= $lead_list_result->num_rows) {
                while ($lead = $lead_list_result->fetch_assoc()) { ?>[
                        ...
                        gridjs.html(`<?php echo ($lead['lead_delivery_date'] == "0000-00-00" || empty($lead['lead_delivery_date'])) ? "00-00-0000" : date("d-m-Y", strtotime($lead['lead_delivery_date'])); ?>`),
                        gridjs.html(`<?php echo ($lead['assign_technician_date'] == "0000-00-00" || empty($lead['assign_technician_date'])) ? "00-00-0000" : date("d-m-Y", strtotime($lead['assign_technician_date'])); ?>`),
