...
if (isset($_POST['delete_lead'])) {
    ...
    if ($checkDelete) {
        header("Location: followup-missed-list.php");
        exit;
    } else {
...
$endDate->modify('-1 day'); // Up to yesterday
$start = $startDate->format('Y-m-d');
$end = $endDate->format('Y-m-d');
$leadData = [];
$lead_list_query = "SELECT * FROM `leads` WHERE `lead_for_company` = '$Company' AND `lead_for_branch` = '$Branch' AND `lead_assign_to` != '' AND `lead_updatedby` = '' AND `lead_type` NOT IN ('Completed', 'Cancel') AND `lead_next_followup_date` BETWEEN '$start' AND '$end'ORDER BY `lead_next_followup_date` DESC, `id` DESC";
        ...
        const leadData = [
            <?php $counterValue = 1;
                foreach ($leadData as $lead) { ?>[
                        ...
                        gridjs.html(`<?php echo $lead['lead_lemark']; ?>`)
                    ],
            <?php } ?>
        ];
