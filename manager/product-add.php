...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['new_product'])) {
        ...
        if (mysqli_num_rows($checkProduct) > 0) {
            ...
            $NewProductQuery = "INSERT INTO `products`(`product_id`, `product_name`, `product_varient`, `product_short_description`, `product_for_company`, `product_price`, `status`, `product_createdby`) 
                VALUES ('$product_id', '$product_name', '$product_varient', '$product_short_description', '$product_for_company', '$product_price', '$product_status', '$product_createdby')";
            $ApplyAddQuery = mysqli_query($conn, $NewProductQuery);

            if ($ApplyAddQuery) {
                ################# Push activity ################
                $activity_id = $guid;
                $activity_content = "New product " . $product_name . " has been added in product list by manager " . $username . " of " . $Branch . " branch.";
                $activity_type = 'Alert';
