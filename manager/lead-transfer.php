...
$lead_list = "SELECT * FROM `leads` WHERE `lead_for_branch` = '$Branch' AND `lead_assign_to` != '' AND `lead_updatedby` != '' AND `lead_customer_status` = '1' ORDER BY `id` DESC";
...
if (isset($_POST['transfer_lead'])) {
    ...
    ################# Push activity ################
    $activity_id = $guid;
    $activity_content = "Some leads transfer to staff " . $lead_assign_to . " has beed transferd by manager " . $username . " of " . $Branch . " branch.";
    $activity_type = 'Alert';
