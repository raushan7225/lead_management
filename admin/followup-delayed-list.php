...
if (isset($_POST['delete_lead'])) {
    ...
    $applyDeleteQuery = mysqli_query($conn, $deleteLeadQuery);
    ...
    if ($applyDeleteQuery) {
        ...
        ################# Push activity ################
        header("Location: followup-delayed-list.php");
        exit();
    } else {
        echo "Error: " . mysqli_error($conn);
    }
} elseif (isset($_POST['edit_lead'])) {
    header("Location: followup-edit.php?id=" . $_POST['mylead_id']);
    exit();
}
############### Edit / Delete ###############
...
$lead_filter_query = "SELECT * FROM `leads` 
    ...
    AND `lead_next_followup_date` BETWEEN '$startDate' AND '$endDate'
