...
if (isset($_GET['lead_for_branch'])) {
    ...
    $lead_list = "SELECT 
        ...
        FROM `leads` WHERE `lead_for_company` = '$Company' AND `lead_for_branch` = '$lead_for_branch' AND `lead_assign_to`!='' AND `lead_customer_status` = '1'";
    ...
    $lead_list = "SELECT 
        ...
        COUNT(CASE WHEN `lead_type` = 'Completed' THEN 1 END) AS complete_count,
        COUNT(CASE WHEN `lead_type` = 'Place Order' THEN 1 END) AS place_order_count
        FROM `leads` WHERE `lead_for_company` = '$Company' AND `lead_assign_to`!='' AND `lead_customer_status` = '1'";
