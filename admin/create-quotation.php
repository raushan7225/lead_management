...
if (isset($_GET['id'])) {
    ...
    ################### Show Leads Details ########################
    $checkLeads = "SELECT * FROM `leads` WHERE `lead_for_company`='$Company' AND `lead_customer_status` = '1' AND `id` = '$id'";
        ...
        while ($lead = mysqli_fetch_assoc($checkLeadsQuery)) {
            ...
            foreach ($lead_product_services as $key => $leadService) {
                $myServiceLead[] = [
                    ...
                    'lead_product_regular_price' => isset($lead_product_regular_price[$key]) ? $lead_product_regular_price[$key] : '',
                    'lead_product_quantity' => isset($lead_product_quantity[$key]) ? $lead_product_quantity[$key] : '',
                    'lead_product_offer_price' => isset($lead_product_offer_price[$key]) ? $lead_product_offer_price[$key] : '',
                    'lead_product_remark' => isset($lead_product_remark[$key]) ? $lead_product_remark[$key] : '',
                ];
            }


            ######### Fetch all employees from the database #########
            $Employee_list = "SELECT * FROM `employees` WHERE `employee_of_company`='$Company' AND `employee_of_branch`='$lead_for_branch' AND `employee_user_role` NOT IN ('Developer', 'Super Admin')  AND `employee_status` = '1'";
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <select name="$assign_technician" id="assign_technician" class="form-control" data-choices>
                                                        ...
                                                        <?php foreach ($Technicians as $Techassign) { ?>
                                                            <option value="<?php echo $Techassign['employee_name']; ?>" <?php echo (isset($Techassign['employee_name']) && $Techassign['employee_name'] == $assign_technician) ? 'selected' : ''; ?>>
...
<?php
    ...
    if (isset($_POST['update_lead'])) {
        ...
        `lead_product_remark`='$product_remarks' WHERE `id` = '$id'";
        ...
        if ($updateLeadQuery) {
            ################# Push activity ################
            $activity_id = $guid;
            $activity_content = "Quotation of customer " . $lead_customer_name . " has been updated by admin " . $username . " of " . $Company . " company.";
            $activity_type = 'Alert';
