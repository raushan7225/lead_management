...
if (isset($_POST['delete_lead'])) {
    ...
    if ($applyDeleteQuery) {
        ...
        ################# Push activity ################
        header("Location: followup-upcoming-list.php");
        exit();
    } else {
    ...
    $mylead_id = $_POST['mylead_id'];
    header("Location: followup-edit.php?id=$mylead_id");
    exit();
}
################# Manage Followup / Delete #################
...
$startDate = (clone $currentDate)->modify('+1 day')->format('Y-m-d');
$endDate = (clone $currentDate)->modify('+365 days')->format('Y-m-d');
