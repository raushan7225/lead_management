...
if (isset($_POST['delete_employee'])) {
     ...
     $deleteEmployeeQuery = "DELETE FROM `employees` WHERE `employee_of_company`='$Company' AND `id` = '$employee_id'";
     $checkDelete = mysqli_query($conn, $deleteEmployeeQuery);

     if ($checkDelete) {
          ################# Push activity ################
          $activity_id = $guid;
          $activity_content = "Employee " . $employee_name . " has been removed by admin " . $username . " of " . $Company . " company.";
          $activity_type = 'Alert';
...
if (isset($_GET["employee_user_role"])) {
     $employee_user_role = $_GET["employee_user_role"];
     $employees_list = "SELECT * FROM `employees` WHERE `employee_of_company`='$Company' AND `employee_user_role`='$employee_user_role' ORDER BY `id` DESC";
     $employees_list_result = $conn->query($employees_list);
} else {
     $employees_list = "SELECT * FROM `employees` WHERE `employee_of_company`='$Company' AND `employee_user_role` NOT IN ('Admin', 'Super Admin') ORDER BY `id` DESC";
     $employees_list_result = $conn->query($employees_list);
}
