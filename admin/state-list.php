...
if (isset($_POST['delete_state'])) {
     ...
     if ($checkDelete) {
          ...
          ################# Push activity ################
          $activity_id = $guid;
          $activity_content = "State " . $state_name . " has been removed by admin " . $username . " of " . $Company . " company.";
          $activity_type = 'Alert';
...
if (isset($_POST["add_state"])) {
     ...
     if (mysqli_num_rows($checkStateQuery) > 0) {
          ...
          if ($add_state_query) {
               ...
               ################# Push activity ################
               $activity_id = $guid;
               $activity_content = "New state " . $state_name . " has been added by admin " . $username . " of " . $Company . " company.";
               $activity_type = 'Alert';
               $activity_company = $Company;
               $activity_branch = $Branch;
               $activity_on = date('Y-m-d H:i:s');
