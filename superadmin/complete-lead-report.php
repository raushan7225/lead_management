...
$lead_filter = "SELECT * FROM `leads` WHERE 1=1";
######### Fetch all leads from the database #########


############### Apply filters based on POST request (search form submission) ##############
if (isset($_POST['search_lead'])) {
    $lead_followup_start_date = $_POST['lead_followup_start_date'];
    $lead_followup_end_date = $_POST['lead_followup_end_date'];
    $lead_for_company = $_POST['lead_for_company'] ?? '';
    $lead_for_branch = $_POST['lead_for_branch'] ?? '';
    $lead_updatedby = $_POST['lead_updatedby'] ?? '';
    ...
    if (!empty($lead_for_company)) {
        $urlParams[] = "lead_for_company=" . urlencode($lead_for_company);
    }
    if (!empty($lead_for_branch)) {
        $urlParams[] = "lead_for_branch=" . urlencode($lead_for_branch);
    }
    if (!empty($lead_updatedby)) {
        $urlParams[] = "lead_updatedby=" . urlencode($lead_updatedby);
    }
...
$lead_followup_start_date = isset($_GET['lead_followup_start_date']) ? $_GET['lead_followup_start_date'] : null;
$lead_followup_end_date = isset($_GET['lead_followup_end_date']) ? $_GET['lead_followup_end_date'] : null;
$lead_for_company = isset($_GET['lead_for_company']) ? $_GET['lead_for_company'] : null;
$lead_for_branch = isset($_GET['lead_for_branch']) ? $_GET['lead_for_branch'] : null;
$lead_updatedby = isset($_GET['lead_updatedby']) ? $_GET['lead_updatedby'] : null;
...
if (!empty($lead_for_company)) {
    $lead_filter .= " AND `lead_for_company` = '" .  $lead_for_company . "'";
}
if (!empty($lead_for_branch)) {
    $lead_filter .= " AND `lead_for_branch` = '" .  $lead_for_branch . "'";
}
if (!empty($lead_updatedby)) {
    $lead_filter .= " AND `lead_updatedby` = '" .  $lead_updatedby . "'";
}
if (!empty($lead_customer_name)) {
...
if (!empty($lead_filter_result) || !empty($lead_followup_end_date) || !empty($lead_for_company) || !empty($lead_for_branch) || !empty($lead_updatedby) || !empty($lead_customer_name) || !empty($lead_type)) {
    $lead_filter_result = $conn->query($lead_filter);
} else {
...
<head>
    ...
    <style>
        ...
        .bootstrap-table .table thead th:nth-child(21) .th-inner,
        .bootstrap-table .table thead th:nth-child(22) .th-inner,
        .bootstrap-table .table thead th:nth-child(23) .th-inner {
            width: 150px !important;
        }
                ...
                <div class="row">
                                    ...
                                    <div class="card">
                                        ...
                                        <div class="card-body">
                                            <div class="row">
                                                ...
                                                <div class="col-lg-3">
                                                    <div class="mb-3">
                                                        <label for="lead_updatedby" class="form-label">Handled by</label>
                                                        <select class="form-control" id="lead_updatedby" name="lead_updatedby" data-choices>
                                                            <option value="">All Employees</option>
                                                            <?php foreach ($Employeename as $employee): ?>
                                                                <option value="<?php echo htmlspecialchars($employee['employee_username']); ?>"
                                                                    <?php echo (isset($lead_updatedby) && $employee['employee_username'] == $lead_updatedby) ? 'selected' : ''; ?>>
                    ...
                    <div class="col-xl-12">
                        <div class="card">
                            ...
                            <div class="card-body">
                                <table id="table" data-show-columns="true" data-show-export="true" data-show-toggle="true" width="100%" class="text-center"></table>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        $(function() {
            ...
            $('#table').bootstrapTable({
                ...
                columns: [{
                    ...
                    {
                        field: 'lead_for_company',
                        title: 'Lead of Company'
                    },
                    {
                        field: 'lead_for_branch',
                        title: 'Lead of Branch'
                    },
                    {
