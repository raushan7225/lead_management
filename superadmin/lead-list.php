...
if (isset($_POST['mylead_id'])) {
    ...
    if (isset($_POST['manage_followup'])) {
        redirect("manage-followup.php?id=$mylead_id");
    } elseif (isset($_POST['create_qutation'])) {
        redirect("create-quotation.php?id=$mylead_id");
    } elseif (isset($_POST['create_report'])) {
        redirect("create-report.php?id=$mylead_id");
...
$branch_query = "SELECT * FROM `branches` WHERE `branch_status` = '1' ORDER BY `branch_name` ASC";
...
$employee_query = "SELECT * FROM `employees` WHERE `employee_user_role` = 'Staff' AND `employee_status` = '1'";

// If a branch is selected, filter employees by that branch
if (isset($_GET['lead_for_branch']) && $_GET['lead_for_branch'] != '') {
    $lead_for_branch = $_GET['lead_for_branch'];
    $employee_query .= " AND `employee_of_branch` = '$lead_for_branch'";
}
...
if (isset($_POST['search_lead'])) {
    ...
    if (!empty($lead_for_branch)) {
        $urlParams[] = "lead_for_branch=$lead_for_branch";
    }
    ...
    if (!empty($urlParams)) {
        ...
        redirect("lead-list.php?$queryString");
    }
}
############ Apply Filter On Lead List ##############
...
$lead_filter_query = "SELECT * FROM `leads` WHERE `lead_customer_status` = '1'";
...
$lead_filter_result = mysqli_query($conn, $lead_filter_query);
$lead_data = [];
if ($lead_filter_result && mysqli_num_rows($lead_filter_result) > 0) {
    while ($lead = mysqli_fetch_assoc($lead_filter_result)) {
        $lead_data[] = $lead;
    }
}
############# Filter Applied on Lead #################
        ...
        if (document.getElementById("table-lead-search")) {
            new gridjs.Grid({
                columns: [{
                    ...
                    {
                        name: "Lead for Company",
                        width: "250px"
                    },
                    {
                        name: "Lead for Branch",
                        width: "270px"
                    },
                    {
                        name: "Address",
