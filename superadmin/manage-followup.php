...
if (isset($_GET['id'])) {
          ...
          if ($lead_for_branch) {
               ...
               if (mysqli_num_rows($FindCompanyResult) > 0) {
                    while ($CompanyData = $FindCompanyResult->fetch_assoc()) {
                         ...
                         ######### Fetch all products from the database #########
                         $products_list = "SELECT * FROM `products` WHERE `product_for_company`='$MyCompany' AND `status` = '1'";
                         $products_list_result = $conn->query($products_list);
                         $productname = [];
                         if ($products_list_result->num_rows > 0) {
                              while ($ProductData = $products_list_result->fetch_assoc()) {
                         ...
                         ######### Fetch all references from the database #########
                         $reference_list = "SELECT * FROM `references` WHERE `reference_for_company`='$MyCompany' AND `reference_status` = '1'";
                         $reference_list_result = mysqli_query($conn, $reference_list);
                         $reference_name = [];
                         if (mysqli_num_rows($reference_list_result) > 0) {
                              while ($reference = $reference_list_result->fetch_assoc()) {
                         ...
                         $Employees = [];
                         $Technicians = [];
                         $Employee_list = "SELECT * FROM `employees` WHERE `employee_of_company`='$MyCompany' AND `employee_of_branch`='$MyBranch' AND `employee_status` = '1'";
                         $Employee_list_result = $conn->query($Employee_list);
     ...
     if (isset($_POST['update_lead'])) {
          ...
          if (!empty($_POST['assign_technician'])) {
               $assign_technician = $_POST['assign_technician'];
          ...
          $UpdateManFollowups = mysqli_query($conn, $updateLead);


          // Execute query and handle response
          if ($UpdateManFollowups) {
               ################# Push activity ################
               $activity_id = $guid;
               $activity_content = "Followup of the customer " . $lead_customer_name . " has been followed up by " . $Role . ".";
               $activity_type = 'Alert';
               $activity_company = "";
               $activity_branch = "";
                                        ...
                                        <div class="card-body">
                                             <div class="row">
                                                  ...
                                                  <div class="col-lg-12">
                                                       ...
                                                       <textarea type="text" id="lead_remark" name="lead_remark" rows="3" class="form-control" placeholder="Remarks"><?php echo $lead_remark; ?></textarea>
                                                  </div>
