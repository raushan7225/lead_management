...
if ($lead_for_branch) {
     ...
     if (mysqli_num_rows($FindCompanyResult) > 0) {
          while ($CompanyData = $FindCompanyResult->fetch_assoc()) {
               ...
               $Employees = [];
               $Technicians = [];
               $Employee_list = "SELECT * FROM `employees` WHERE `employee_of_company`='$MyCompany' AND `employee_of_branch`='$MyBranch' AND `employee_user_role` NOT IN ('Manager', 'Developer', 'Super Admin') AND `employee_status` = '1'";
               ...
               if (isset($_POST['add_lead'])) {
                    ...
                    $lead_for_company = $MyCompany;
                    $lead_for_branch = $MyBranch;
                    ...
                    if (mysqli_num_rows($checkContactResult) > 0) {
                         ...
                         $LeadAddQuery = mysqli_query($conn, $addLead);

                         if ($LeadAddQuery) {
                              ################# Push activity ################
                              $activity_id = $guid;
                              $activity_content = "The customer " . $lead_customer_name . " has been added by " . $Role . ".";
                              $activity_type = 'Alert';
                              $activity_company = "";
                              $activity_branch = "";
                                                            ...
                                                            <select class="form-control" id="lead_assign_to" name="lead_assign_to" data-choices>
                                                                 ...
                                                                 <?php foreach ($Employees as $employee) { ?>
                                                                      <option value="<?php echo $employee['employee_username']; ?>"
                                                                           <?php echo (isset($employee['employee_username']) && $employee['employee_username'] == $lead_assign_to) ? 'selected' : ''; ?>>
