...
######### Build query for leads with lead_type = 'Cancel' #########
$lead_list_query = "SELECT * FROM `leads` WHERE `lead_type` = 'Cancel' AND `lead_assign_to` != '' AND `lead_updatedby` != ''";
...
if (isset($_GET["lead_for_company"])) {
    ...
    $lead_list_query = "SELECT * FROM `leads` WHERE `lead_for_company` = '$lead_for_company' AND `lead_type` = 'Cancel' AND `lead_assign_to` != '' AND `lead_updatedby` != '' ORDER BY `id` DESC";
    $lead_list_result = $conn->query($lead_list_query);
}
#################### If set lead for company ####################
?>
            ...
            if ($counterValue <= $lead_list_result->num_rows) {
                while ($lead = $lead_list_result->fetch_assoc()) { ?>[
                        ...
                        gridjs.html(`
                            <div class="row gap-2">
                                <div class="col-5">
                                    <form action="#" method="post" enctype="multipart/form-data" data-bs-toggle="tooltip" data-bs-custom-class="primary-tooltip" data-bs-title="View the lead details.">
                                        <input type="hidden" name="mylead_id" value="<?php echo $lead['id']; ?>">
                                        <button type="submit" name="manage_followup" value="<?php echo $lead['id']; ?>" class="btn btn-soft-primary btn-sm"><iconify-icon icon="solar:eye-scan-line-duotone" class="align-middle fs-18"></iconify-icon></button>
                                    </form>
                                </div>
