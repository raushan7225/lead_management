...
if (isset($_POST['search_lead'])) {
    ...
    if (!empty($urlParams)) {
        ...
        echo "<script>window.location.href='followup-upcoming-list.php?$queryString';</script>";
        exit;
    }
}
...
######### Fetch Upcoming Leads (Tomorrow to +365 Days) #########
...
$lead_list_query = "SELECT * FROM `leads` 
    WHERE `lead_next_followup_date` BETWEEN '" . $startDate->format('Y-m-d') . "' AND '" . $endDate->format('Y-m-d') . "'
    AND `lead_assign_to` != '' 
    AND `lead_updatedby` != '' 
...
$lead_list_query .= " ORDER BY `lead_next_followup_date` ASC";

$leadData = [];
$lead_list_result = $conn->query($lead_list_query);
if ($lead_list_result && $lead_list_result->num_rows > 0) {
    $leadData = $lead_list_result->fetch_all(MYSQLI_ASSOC);
} else {
    echo "<p>No upcoming leads found.</p>";
}
################# Apply filters if set ##################
...
if (isset($_GET["lead_for_company"])) {
    ...
    $lead_list_query = "SELECT * FROM `leads` WHERE `lead_for_company` = '$lead_for_company' AND `lead_next_followup_date` BETWEEN '" . $startDate->format('Y-m-d') . "' AND '" . $endDate->format('Y-m-d') . "' AND `lead_assign_to` != ''  AND `lead_updatedby` != ''  AND `lead_type` NOT IN ('Completed', 'Cancel')  AND `lead_customer_status` = '1'";
    $lead_list_result = $conn->query($lead_list_query);
    if ($lead_list_result && $lead_list_result->num_rows > 0) {
        $leadData = $lead_list_result->fetch_all(MYSQLI_ASSOC);
    } else {
...
<head>
    <title>Upcoming Followups</title>
                            ...
                            <div class="col-lg-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h4 class="card-title">Filter Upcoming Followups</h4>
                                    </div>
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-lg-12">
                                                <div class="row justify-content-between g-2">
                                                    ...
                                                    <div class="col-lg-2">
                                                        <a href="followup-upcoming-list.php" class="btn btn-primary w-100">Reset</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
    ...
    <script>
        ...
        const leadData = [
            ...
            foreach ($leadData as $lead) { ?>[
                    ...
                    gridjs.html(`<?php echo $lead_for_company = $lead['lead_for_company'] ?>`),
                    gridjs.html(`<?php echo $lead_for_branch = $lead['lead_for_branch'] ?>`),
