    ...
    if (!empty($mylead_id)) {
        if (isset($_POST['delete_lead'])) {
            ...
            if ($deleteResult) {
                ...
                ################# Push activity ################
                echo "<script>window.location.href='followup-delayed-list.php';</script>";
            } else {
...
if (isset($_POST['search_lead'])) {
    ...
    if (!empty($urlParams)) {
        ...
        echo "<script>window.location.href='followup-delayed-list.php?$queryString';</script>";
        exit;
    }
}
######### End Search Filter Submission #########


######### Apply Date Range + Filter Logic #########
$currentDate = new DateTime();
$startDate = (clone $currentDate)->modify('-365 days')->format('Y-m-d');
$endDate   = (clone $currentDate)->modify('-1 day')->format('Y-m-d');

// Base Query
$lead_list_query = "SELECT * FROM `leads` 
    WHERE `lead_next_followup_date` BETWEEN '$startDate' AND '$endDate'
    AND `lead_assign_to` != ''
    AND `lead_updatedby` = ''
...
$lead_list_query .= " ORDER BY `lead_next_followup_date` DESC, `id` DESC";
...
if (isset($_GET["lead_for_company"])) {
    ...
    $lead_list_query = "SELECT * FROM `leads` WHERE `lead_for_company` = '$lead_for_company' AND `lead_next_followup_date` BETWEEN '$startDate' AND '$endDate' AND `lead_assign_to` != '' AND `lead_updatedby` = '' AND `lead_type` NOT IN ('Completed', 'Cancel')";
    $lead_list_result = $conn->query($lead_list_query);
    if ($lead_list_result && $lead_list_result->num_rows > 0) {
        $leadData = $lead_list_result->fetch_all(MYSQLI_ASSOC);
    }
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-lg-12">
                                                <div class="row justify-content-between g-2">
                                                    ...
                                                    <div class="col-lg-2">
                                                        <a href="followup-delayed-list.php" class="btn btn-primary w-100">Reset</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
