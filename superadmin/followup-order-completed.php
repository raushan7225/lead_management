    ...
    if (!empty($mylead_id)) {
        if (isset($_POST['delete_lead'])) {
            ...
            if ($deleteResult) {
                ...
                ################# Push activity ################
                echo "<script>window.location.href='followup-order-completed.php';</script>";
            } else {
...
if (isset($_POST['search_lead'])) {
    ...
    if (!empty($urlParams)) {
        ...
        echo "<script>window.location.href='followup-order-completed.php?$queryString';</script>";
        exit;
    }
}
######### End filter form #########
...
$lead_list = "SELECT * FROM `leads` WHERE `lead_assign_to` != '' AND `lead_updatedby` != '' AND `lead_type` = 'Completed'";
...
if (isset($_GET["lead_for_company"])) {
    ...
    $lead_list = "SELECT * FROM `leads` WHERE `lead_for_company` = '$lead_for_company' AND `lead_assign_to` != '' AND `lead_updatedby` != '' AND `lead_type` = 'Completed' ORDER BY `id` DESC";
    $lead_list_result = $conn->query($lead_list);
}
#################### If set lead for company ####################
?>
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-lg-12">
                                                <div class="row justify-content-between g-2">
                                                    ...
                                                    <div class="col-lg-2">
                                                        <a href="followup-order-completed.php" class="btn btn-primary w-100">Reset</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
            ...
            if ($counterValue <= $lead_list_result->num_rows) {
                while ($lead = $lead_list_result->fetch_assoc()) { ?>[
                        ...
                        gridjs.html(`
                            <div class="row gap-2">
                                <div class="col-5">
                                    <form action="#" method="post" enctype="multipart/form-data" data-bs-toggle="tooltip" data-bs-custom-class="primary-tooltip" data-bs-title="No Need to Edit followups the lead closed! If you want to edit then click button">
                        ...
                        gridjs.html(`<?php echo date('d-m-Y', strtotime($lead['assign_technician_date'])); ?>`),
                        gridjs.html(`<?php echo $assign_technician = $lead['assign_technician'] ?>`),
