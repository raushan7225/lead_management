...
$sql = "SELECT * FROM `leads` WHERE `lead_for_company` = '$Company'  AND `lead_for_branch` = '$Branch'  AND `assign_technician` = '$username'  AND `lead_delivery_date` < '$currentDate'  AND `lead_delivery_date` >= '$pastDate' AND `lead_instalation_aggreement` = ''  AND `lead_type` = 'Completed'  AND `lead_customer_status` = '1'  ORDER BY `lead_delivery_date` DESC, `id` DESC";
                    ...
                    <div class="col">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title"> Missed Installations </h3>
                            </div>
                            <div class="card-body">
