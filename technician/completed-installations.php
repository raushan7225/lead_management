...
################## Missed Tasks ####################
$sql = "SELECT * FROM `leads` WHERE `lead_for_company` = '$Company'  AND `lead_for_branch` = '$Branch'  AND `assign_technician` = '$username'  AND `lead_instalation_aggreement` != ''  AND `lead_type` = 'Completed' AND `lead_customer_status` = '1'  ORDER BY `id` DESC";
