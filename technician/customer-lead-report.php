...
if (isset($_POST['search_lead'])) {
    ...
    if (!empty($lead_followup_start_date)) {
        $urlParams[] = "starting_date=$lead_followup_start_date";
    }
    if (!empty($lead_followup_end_date)) {
        $urlParams[] = "lead_followup_end_date=$lead_followup_end_date";
    }
...
######### Apply filters from GET (direct access or redirection) #########
$lead_followup_start_date = isset($_GET['starting_date']) ? $_GET['starting_date'] : '';
...
$lead_filter = "SELECT * FROM `leads` WHERE `lead_for_company` = '$Company' AND `lead_for_branch` = '$Branch' AND `assign_technician` = '$username'";
