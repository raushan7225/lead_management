...
if (isset($_POST['delete_lead'])) {
    ...
    if ($checkDelete) {
        echo "<script>window.location.href='followup-today-visit.php'</script>";
    } else {
...
$date = date("Y-m-d");
$lead_list = "SELECT * FROM `leads` WHERE `lead_for_company` = '$Company' AND `lead_for_branch` = '$Branch' AND `lead_next_followup_date`='$date' AND `lead_assign_to`= '$username' AND `lead_updatedby`='$username' AND `lead_status` = 'Client Visit' AND `lead_customer_status`='1' ORDER BY `id` DESC";
